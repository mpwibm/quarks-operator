...
var _ = Describe("K8s native resources provide BOSH links to a BOSH deployment", func() {
	...
	jobLink := func() manifest.JobLink {
		...
		Expect(err).ToNot(HaveOccurred())
		testing.JSONDump(manifest)
		Expect(manifest.InstanceGroups).To(HaveLen(1))
		Expect(manifest.InstanceGroups[0].Jobs).To(HaveLen(1))
		return manifest.InstanceGroups[0].Jobs[0].Properties.Quarks.Consumes["quarks-gora"]
	}

	FContext("when the link has an underscore in its name", func() {
		BeforeEach(func() {
